...
class RunControl:
    ...
    def LoopRunControll(self, key='', dato='', name=''):
        ...
        # ---- Selezione CMD 15
            ...
            try:
                ...
                pass
            if fdato <= 2555:
                self.timeout = int(fdato / 5)
            else:
                self.timeout = 200
            self.Port.Send_CMD(0xA, self.timeout << 3)
        elif key == '16':
            if dato == 'D':
                self.Port.Send_CMD(0xC, 0x4)
            elif dato == 'E':
                self.Port.Send_CMD(0xC, 0x0)
            else:
                self.Port.Send_CMD(0xC, 0x0)
        # Salvataggio dati
        elif key == 'slog':
        ...
        # salvataggio configurazione
            ...
            if int(config['DEFAULT']['Timeout']) <= 2555:
                self.timeout = int(int(config['DEFAULT']['Timeout']) / 5)
            else:
                self.timeout = 200
            self.Port.Send_CMD(0xA, (self.timeout << 3))
    ...
    def EstrazioneParametri(self, Cmd, Valore):
        ...
        if list(self.DatiLetture.items())[0][0] == Cmd:
            ...
            if binNum[15] == "0":
                self.Overcurrent0 = 'Troppa corrente'
            elif binNum[15] == "1":
                self.Overcurrent0 = 'OK'
            else:
                pass

            if binNum[14] == "0":
                self.Overcurrent1 = 'Troppa corrente'
            elif binNum[14] == "1":
                self.Overcurrent1 = 'OK'
            else:
                pass
        ...
        # - codifica cmd CLK reg
            ...
            binNum = '{0:014b}'.format(int(Valore))
            if binNum[11:] == "001":
                ...
                pass
            self.timeout = int(binNum[:11], 2) * 5
